...
class MyApplication : Application() {
    ...
    override fun onCreate() {
        super.onCreate()
        Lingver.init(this@MyApplication, "en")
            ...
            ref.child(uid)
                .addValueEventListener(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        //get user info
                        val language = snapshot.child("language").value as? String
                        val mode = snapshot.child("mode").value as? String
                        if (language != null ) {
                            Lingver.getInstance().setLocale(this@MyApplication, language)
                        } else {
                            Lingver.init(this@MyApplication, "en")
                        }
                        if (mode == null ){
